...
int main() {
    ...
    // Mode and temp for the user.
    int mode, temp;

    // Key to search and remove functions.
    int key;

    // Key for the 2nd print function.
    int key_chain;

    // Divisor used by the hash function and to create the table by determining the table size.
    int divisor;
...
// for taking input students. Do you know why?
    ...
    // Read in the user data and create the table.
    cin >> mode >> key >> key_chain >> divisor;
    x.CreateTable(divisor);
    while (cin >> temp) {
        Student.Set_key(temp); // New student is created with a key assigned.
        x.Add(Student); // Student is added to the table.
    }
