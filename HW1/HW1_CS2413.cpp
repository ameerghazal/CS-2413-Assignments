...
// Complete the header files by importing an IO stream and defining the name space
...
//    Loop over the array and assign each sooner index from the students list.
    for (int i = 0; i < 5; ++i) {
        Sooner[i] = x[i];
    }
}
...
float University::GPA_Mean() {
    ...
    for (int i = 0; i < 5; ++i) {
        mean += Sooner[i].get_GPA(); // total of all the gpas
    }

    cout << (mean / 5); // divide by n and return.
    return (mean / 5);
}
...
float University::GPA_Min() {
    ...
    float min = Sooner[0].get_GPA(); // store the minimum as the first value.
    for (int i = 0; i < 5; i++) { // loop over and checker for lower values.
...
// Constructor that initalizes all the indices of the sooner array to -1.
    for (int i = 0; i < 5; ++i) {
        Sooner[i].set_SID(-1);
        Sooner[i].set_GPA(-1);
    }
