...
Node* AVL::Add(Node *p) {
    ...
    Node* temp = Search(p->Get_key());

    // 1.5): Check if the node is already contained within the AVL; if so, return the address.
    if (temp->Get_key() == p->Get_key()) return p;

    // 2): Otherwise, now, connect the links and add the node.
    p->Set_parent(temp);

    if (p->Get_key() < temp->Get_key()) temp->Set_left(p);
    else if (p->Get_key() > temp->Get_key()) temp->Set_right(p);
...
Node* AVL::Remove(int key) {
    ...
    if (temp->Get_key() != key) return nullptr;
